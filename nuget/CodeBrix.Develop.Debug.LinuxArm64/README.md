# CodeBrix.Develop.Debug.LinuxArm64

A debugger for the .NET runtime, packaged as **linux-arm64 native binaries**. It implements the [GDB/MI](https://sourceware.org/gdb/onlinedocs/gdb/GDB_002fMI.html) and [VSCode Debug Adapter Protocol](https://microsoft.github.io/debug-adapter-protocol/) interfaces, so an application can debug .NET programs by launching the debugger as a child process and speaking either protocol over stdio.

This package supports **linux-arm64 only** — it is a carrier for native binaries, contains no managed library, and adds no NuGet dependencies.

CodeBrix.Develop.Debug.LinuxArm64 supports applications and assemblies that target Microsoft .NET version 10.0 and later.
...
## CodeBrix.Develop.Debug.LinuxArm64 supports:
...
* Automatic copy of the debugger into the consuming project's output folder, on linux-arm64.
...
CodeBrix.Develop.Debug.LinuxArm64 is licensed under the MIT License - see the
