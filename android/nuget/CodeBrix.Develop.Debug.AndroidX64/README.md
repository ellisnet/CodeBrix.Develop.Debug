...
## CodeBrix.Develop.Debug.AndroidX64 supports:
...
var payload = Path.Combine(AppContext.BaseDirectory, "netcoredbg-android-x64");
...
| File | Purpose |
|---|---|
| `netcoredbg` | The native debugger executable (android-x64), with the Android compatibility layer built in |
| `libdbgshim.so` | The debugger shim that finds the app's runtime and loads its debugging interface (android-x64) |
...
CodeBrix.Develop.Debug.AndroidX64 is licensed under the MIT License - see the
